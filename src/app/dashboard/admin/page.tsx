...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1c2e] via-[#2d1b4e] to-[#0f0c29] w-full p-6 pl-0 md:pl-6">
      <div className="w-full max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          {/* Header */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <div className="flex-1">
              <h1 className="text-4xl font-bold text-white mb-2" style={{ fontFamily: 'var(--font-uni-salar)' }}>بەڕێوەبردنی سیستەم</h1>
              <p className="text-gray-400" style={{ fontFamily: 'var(--font-uni-salar)' }}>بەڕێوەبردنی بەکارهێنەران، ڕۆڵەکان و ڕێکخستنەکان</p>
            </div>
            <StatCards users={users} roles={roles} />
          </div>

          {/* Tab Navigation */}
          <div className="flex flex-row overflow-x-auto whitespace-nowrap gap-2 mb-8 bg-[#2a2d3e]/60 backdrop-blur-xl border border-white/10 shadow-xl rounded-2xl p-2">
            <button onClick={() => setActiveTab("users")} className={`flex-1 py-3 px-6 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 ${activeTab === "users" ? "bg-blue-500 text-white shadow-lg" : "text-gray-300 hover:bg-white/10"}`} style={{ fontFamily: 'var(--font-uni-salar)' }}>
              <FaUsers /> بەکارهێنەران
            </button>
            <button onClick={() => setActiveTab("roles")} className={`flex-1 py-3 px-6 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 ${activeTab === "roles" ? "bg-purple-500 text-white shadow-lg" : "text-gray-300 hover:bg-white/10"}`} style={{ fontFamily: 'var(--font-uni-salar)' }}>
              <FaShieldAlt /> ڕۆڵەکان
            </button>
            <button onClick={() => setActiveTab("settings")} className={`flex-1 py-3 px-6 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 ${activeTab === "settings" ? "bg-green-500 text-white shadow-lg" : "text-gray-300 hover:bg-white/10"}`} style={{ fontFamily: 'var(--font-uni-salar)' }}>
              <FaCog /> ڕێکخستنەکان
            </button>
          </div>

...
UPDATE sales SET status = 'delivered' WHERE status IS NULL OR status NOT IN ('delivered', 'cancelled', 'refunded', 'pending', 'completed');

-- Finally, add the updated constraint with 'cancelled' as a valid status
ALTER TABLE sales ADD CONSTRAINT sales_status_check
CHECK (status IN ('delivered', 'cancelled', 'refunded', 'pending', 'completed'));
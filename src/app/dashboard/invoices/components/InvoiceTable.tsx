        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-white/10 bg-[#2a2d3e]/60">
              <th className="px-2 md:px-6 py-3 md:py-4 text-center font-semibold text-gray-200 text-xs md:text-sm" style={{ fontFamily: 'var(--font-uni-salar)' }}>
                ژمارەی فاکتور
              </th>
              <th className="px-2 md:px-6 py-3 md:py-4 text-center font-semibold text-gray-200 text-xs md:text-sm" style={{ fontFamily: 'var(--font-uni-salar)' }}>
                کڕیار
          ...
          <tbody>
            {filteredInvoices.map((invoice, index) => (
              ...
              >
                <td className="px-2 md:px-6 py-3 md:py-4 text-gray-200 font-bold text-center text-xs md:text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>
                  #{invoice.invoice_number}
                </td>
                <td className="px-2 md:px-6 py-3 md:py-4 text-gray-200 text-center text-xs md:text-sm" style={{ fontFamily: 'var(--font-uni-salar)' }}>
                  {invoice.customer_name}
                </td>
                <td className="px-2 md:px-6 py-3 md:py-4 text-gray-400 text-center text-xs md:text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>
                ...
                <td className="px-2 md:px-6 py-2 md:py-4">
                  <span className={`px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-medium ${
                    invoice.status === 'completed' ? 'bg-green-500/20 text-green-300' :
                    invoice.status === 'refunded' ? 'bg-red-500/20 text-red-300' :
                    invoice.status === 'cancelled' ? 'bg-gray-500/20 text-gray-300' :
                ...
                <td className="px-2 md:px-6 py-3 md:py-4 text-center">
                  <span className={`px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-medium ${
                    invoice.status === 'cancelled' || invoice.status === 'refunded' ? 'bg-gray-500/20 text-gray-300' :
                    invoice.payment_method === 'cash' ? 'bg-green-500/20 text-green-300' :
                    invoice.payment_method === 'fib' ? 'bg-blue-500/20 text-blue-300' :
                    'bg-orange-500/20 text-orange-300'
                  }`}>
                ...
                <td className="px-2 md:px-6 py-4">
                  <div className="flex flex-row-reverse gap-2 items-start justify-center">
                    ...
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <span className="text-[10px] text-gray-400" style={{ fontFamily: 'var(--font-uni-salar)' }}>چاپکردن</span>
                    </div>
